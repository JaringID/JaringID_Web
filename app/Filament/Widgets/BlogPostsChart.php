...
class BlogPostsChart extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        $pendapatanData = Pendapatan::whereHas('farm', function ($query) use ($userId) {
            ...
            ->select('tanggal', 'pendapatan') // Mengambil data sesuai tanggal
            ->orderBy('tanggal', 'asc')
            ->get();
        ...
        // Memformat data untuk grafik pendapatan
        ...
        $pendapatanMapped = collect($allLabels)->map(function ($tanggal) use ($pendapatanData) {
            return $pendapatanData->where('tanggal', $tanggal)->sum('pendapatan') ?? 0; // Mengambil nilai pendapatan per tanggal
        })->toArray();
